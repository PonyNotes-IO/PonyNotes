...
/// 独立的PDF处理器 - 不依赖Flutter
/// 使用纯Dart实现，可以直接运行测试
class StandalonePdfProcessor {
  /// 处理PDF字节数据
  static Future<String> processPdfBytes(Uint8List bytes) async {
    try {
      ...
      print('StandalonePdfProcessor 错误: $e');
...
Future<void> main(List<String> args) async {
  ...
  if (!await pdfFile.exists()) {
    ...
    print('\n使用方法:');
    print('dart standalone_pdf_processor.dart [PDF文件路径]');
    return;
  }
  ...
  try {
    ...
    try {
      final result = await StandalonePdfProcessor.processPdfBytes(bytes);
      stopwatch.stop();
      ...
      // 保存结果到文件
      final outputFile = File('standalone_pdf_result.md');
      await outputFile.writeAsString(result);
      print('💾 完整结果已保存到: ${outputFile.path}');
      
      // 生成统计信息
      _printStatistics(result);
...
/// 打印统计信息
void _printStatistics(String content) {
  print('');
  print('📈 内容统计:');
  print('-' * 30);
  ...
  // 检查是否包含表格标识
  final hasTable = content.contains('|') || 
                  content.contains('表') || 
                  content.contains('Table') ||
                  RegExp(r'\d+\.\d+').hasMatch(content);
  print('可能包含表格: ${hasTable ? "是" : "否"}');
  
  // 检查是否包含标题
  final hasHeaders = content.contains('#') || 
                    RegExp(r'^[一二三四五六七八九十]+[、.]').hasMatch(content) ||
                    RegExp(r'^\d+[、.]').hasMatch(content);
  print('可能包含标题: ${hasHeaders ? "是" : "否"}');
